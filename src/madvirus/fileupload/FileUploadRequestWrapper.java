...
public class FileUploadRequestWrapper extends HttpServletRequestWrapper {
	...
	private void parsing(HttpServletRequest request, int threshold, int max, String repositoryPath) throws FileUploadException, UnsupportedEncodingException{
		if(FileUpload.isMultipartContent(request)) {
			...
			if(threshold != -1) {
				disFileUpload.setSizeThreshold(threshold);
			}
			disFileUpload.setSizeMax(max);
			if(repositoryPath != null) {
				disFileUpload.setRepositoryPath(repositoryPath);
			}
			
			java.util.List list = disFileUpload.parseRequest(request);
